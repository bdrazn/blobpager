...
llm_graph_result * llama_context::process_ubatch(const llama_ubatch & ubatch, llm_graph_type gtype, llama_memory_context_i * mctx, ggml_status & ret) {
    ...
    if (!graph_reuse_disable && gf_res_prev_active == res && res->can_reuse(gparams)) {
        ...
        if (llama_blobpager_active()) {
            // hybrid: the wrapper adds the CPU miss pass and chains to the user's cb_eval
            llama_blobpager_set_user_cb(cparams.cb_eval, cparams.cb_eval_user_data);
            ggml_backend_sched_set_eval_callback(sched.get(), llama_blobpager_cb_eval, nullptr);
        } else {
