...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    // blobpager hybrid (E7b): route the expert matmuls through the 97-slot GPU
    // pools (hits) with misses landing on the zero dummy slot; the missed
    // experts are computed on the CPU by the eval-callback wrapper and added
    // back through the correction tensor at the end of this function.
    const int64_t bp_limit = il >= 0 ? hparams.swiglu_clamp_exp[il] : 0.0f;
    bool bp_hybrid = llama_blobpager_hybrid(il, n_tokens, norm_w, w_scale)
        && exp_probs_b      == nullptr
        ...
        && !(bp_limit > 1e-6f);
    ggml_tensor * bp_ids  = selected_experts;
    ggml_tensor * bp_up   = up_exps;
    ggml_tensor * bp_gate = gate_exps;
    ggml_tensor * bp_down = down_exps;
    if (bp_hybrid) {
        cb(cur, "blobpager_moe_in", il);
        bp_ids  = llama_blobpager_remap(ctx0, selected_experts, il);
        bp_up   = llama_blobpager_pool(il, 0);
        bp_gate = llama_blobpager_pool(il, 1);
        bp_down = llama_blobpager_pool(il, 2);
    }
    ...
    if (gate_up_exps) {
        ...
        up = build_lora_mm_id(bp_up, cur, bp_ids, up_exps_s); // [n_ff, n_expert_used, n_tokens]
        cb(up, "ffn_moe_up", il);
        ...
        if (gate_exps) {
            cur = build_lora_mm_id(bp_gate, cur, bp_ids, gate_exps_s); // [n_ff, n_expert_used, n_tokens]
            cb(cur, "ffn_moe_gate", il);
        } else {
            cur = up;
        }
    ...
    experts = build_lora_mm_id(bp_down, cur, bp_ids, down_exps_s); // [n_embd, n_expert_used, n_tokens]
    ...
    if (bp_hybrid) {
        // add the CPU-computed miss contributions (written by the eval
        // callback before this node evaluates — the top-k stop point
        // guarantees the correction tensor is already filled)
        moe_out = ggml_add(ctx0, moe_out, llama_blobpager_correction_view(ctx0, il, n_tokens));
    }
